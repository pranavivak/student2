...
- 1.1.1.1.1 no because it does not contain 8 bit numbers. - 23.23.23.23 yes because it has 8 bit numbers seperated into 4 sections by periods. - 134.492.100.0 no because it is contains numbers that are greater than 255. - 255.256.55.255 no because it is contains numbers that are greater than 255. - 2.93.255.19 yes becauuse it contains 8 bit numbers and is in the range of 0-255.
...
- Multicast - Unicast - Broadcast Broadcast because this is when data is sent from one sender to all possible recievers in the network.
Models
...
USB (permits data exchange between electronics) FTP and TLS and ASCII are on the same layer (application) because it focuses on user interactions.
Telnet is a internet protocol which allows remote access to other computers over a local network or the internet. What layer of the OSI model would this protocol be located on? What is the function of this layer? The application layer because its responsible for the network services between user applications.

DNS
Bob wants to use the domain bob.is.the.best.com. What domain should he buy from a DNS provider (assume it is available)? What would be the subdomains? the main domain would be best.com, subdomain is bob.is.the.
HTTP and HTTPS
What is a difference between HTTP and HTTPS? The difference is the level of security as HTTPS has a secure communication protocol.

- What protocol does HTTPS use that HTTP doesn't? The protocol is TLS that HTTPS uses.
Last trimester we sent HTTP requests for our passion projects

- Did we use HTTP or HTTPS? - What are the benefits and disadvantages of this?
TCP and UDP
Bob is setting up a video streaming service, and he needs the stream to be real time.

- What protocol should he use, TCP or UDP? Why? He should use UDP because it has lower latency. - What are some cons of this protocol? Give an example of a potential issue. The cons of this protocol is that it is not as reliable and has no error recovery. An example of this issue is that when a packet is lost there is no mechanism to retrive it.
TCP has error checking, which ensures that all packets arrive properly. Why is this important? This is important because ensures reliable and accurate data transmission.

- Give an example of how TCP ensures that there are no errors. when downloading a file using TCP, each part of the file will be sent in individual packets and TCP checks for errors.
...
- How does Server B ensure that they have received any sent packets before Server A continues sending packets in TCP? In UDP? Server B sends ACK to show that it has successfully recived packet. - What is another use of this? In web applications, the integrity of the data is important so the TCP allows for reliable and error checking manners.